...
# ── AMCL / TF Config ─────────────────────────────────────────────────────────
MAP_FRAME            = 'map'                    # global frame published by AMCL
BASE_FRAME           = f'{NAMESPACE}/base_link' # robot body frame
INITIAL_POSE_DELAY_S = 2.0  # seconds to wait before publishing initial pose (let AMCL start)
...
class SearchAndNavigate(Node):
    ...
    def __init__(self):
        ...
        # AMCL initial pose publisher — seeds particle filter at (0,0) facing +x
        self.initial_pose_pub = self.create_publisher(
            PoseWithCovarianceStamped, f'{NAMESPACE}/initialpose', 10)

        # Subscriptions — no Odometry; pose comes from AMCL via TF2
        ...
        # TF2 — reads AMCL-corrected map→base_link transform each tick
        ...
        # Pose state — updated each tick from AMCL via TF2
        self.current_x   = 0.0
        self.current_y   = 0.0
        ...
        # Publish initial pose once after AMCL has had time to start
        self._initial_pose_sent = False
        self.create_timer(INITIAL_POSE_DELAY_S, self._publish_initial_pose_once)
    ...
    def _publish_initial_pose_once(self):
        """Seed AMCL particle filter at the tape-mark origin (0, 0) facing +x.

        Robot always starts here, so covariance can be kept tight.
        This fires once after INITIAL_POSE_DELAY_S to give AMCL time to start.
        """
        ...
        msg.pose.pose.orientation.z = 0.0  # facing +x → yaw = 0 → qz = 0, qw = 1
        msg.pose.pose.orientation.w = 1.0
        # Covariance diagonal: [xx, yy, zz, rr, pp, yawyaw] at indices 0,7,14,21,28,35
        msg.pose.covariance[0]  = 0.05   # x uncertainty (m²)
        msg.pose.covariance[7]  = 0.05   # y uncertainty (m²)
        msg.pose.covariance[35] = 0.05   # yaw uncertainty (rad²)
        self.initial_pose_pub.publish(msg)
        self._initial_pose_sent = True
        self.get_logger().info('AMCL initial pose published at (0.0, 0.0) yaw=0')
    ...
    def update_pose_from_tf(self):
        """Read AMCL-corrected robot pose from the map → base_link TF transform."""
        try:
            ...
            # AMCL not yet converged or TF not yet available — keep last known pose
            pass
    ...
    def control_loop(self):
        self.update_pose_from_tf()  # refresh AMCL-corrected pose each tick
